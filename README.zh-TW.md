...
[**Tabby**](https://tabby.sh) (前身是 **Terminus**) 是一個可高度個人化的終端模擬器和 SSH 或 Serial 客戶端，支持 Windows，macOS 和 Linux

* 整合 SSH，Telnet 客戶端和連接管理器
* 整合 Serial Port
* 可自訂主題和配色
* 可自訂快捷鍵與多鍵快捷鍵
* 分體式窗格
...
* 在 SSH Session 中通過 Zmodem 進行直接文件傳輸
* 完整的 Unicode 支持，包括雙角字符
* 不會因快速的輸出而卡住
* Windows 上舒適的 shell 體驗，包括 tab 自動補齊（通過 Clink）
* 為 SSH secrets 和設定整合加密容器
* SSH、SFTP 和 Telnet 客戶端可用作 [Web 應用](https://tabby.sh/app)（也可[託管](https://github.com/Eugeny/tabby-web)）

# 目錄 <!-- omit in toc -->

- [Tabby是什麼](#tabby是什麼)
- [Terminal 特色](#terminal-特色)
- [SSH 客戶端](#ssh-客戶端)
...
* **Tabby 不是**一個全新的 shell，也不是 MinGW 或 Cygwin 的替代品。它也不是輕量級的 - 如果你對記憶體使用量很敏感，請考慮 [Conemu](https://conemu.github.io) 或 [Alacritty](https://github.com/jwilm/alacritty)
<a name="terminal"></a>
...
如果在 Tabby.exe 所在的目錄建立一個名為`data`文件夾，Tabby 將可以在 Windows 上作為綠色軟體運行。

<a name="plugins"></a>

# 外掛

外掛和主題可以直接在 Tabby 設置中安裝。
...
* [clippy](https://github.com/Eugeny/tabby-clippy) - 一個可以一直煩你的示例外掛
...
請參閱 [HACKING.md](https://github.com/Eugeny/tabby/blob/master/HACKING.md) 和 [API 文檔](https://docs.tabby.sh/) 以獲取開發檔案 Layout 與外掛開發教學資訊。

---
<a name="contributors"></a>

讓我們謝謝這些熱心貢獻的朋友 ([emoji key](https://allcontributors.org/docs/en/emoji-key)):
...
本項目遵循 [all-contributors](https://github.com/all-contributors/all-contributors) . 歡迎大家貢獻一己之力 !